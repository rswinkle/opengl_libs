...
void GLSLProgram::setUniform( const char *name, const mat4 & m)
{
    ...
    if( loc >= 0 )
    {
        glUniformMatrix4fv(loc, 1, GL_FALSE, (GLfloat*)&m[0][0]);
    } else {
        printf("Uniform: %s not found.\n",name);
    }
}
...
void GLSLProgram::setUniform( const char *name, const mat3 & m)
{
    ...
    if( loc >= 0 )
    {
        glUniformMatrix3fv(loc, 1, GL_FALSE, (GLfloat*)&m[0][0]);
    } else {
        printf("Uniform: %s not found.\n",name);
    }
}
...
void compileAndLinkShader(GLSLProgram& prog, int num_shaders, ...)
{
	va_list shader_list;
	...
	for (int i=0; i<num_shaders; ++i) {
		...
		if( ! prog.compileShaderFromFile(file, GLSLShader::GLSLShaderType(type)) )
		{
